...
# INTSOC_TRANSACTOR ABI/FFI Documentation
...
│  generated/abi/intsoc_transactor.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libintsoc_transactor.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── intsoc_transactor.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── intsoc_transactor.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/intsoc_transactor.h
```
...
```c
#include "intsoc_transactor.h"

int main() {
    void* handle = intsoc_transactor_init();
    if (!handle) return 1;

    int result = intsoc_transactor_process(handle, 42);
    if (result != 0) {
        const char* err = intsoc_transactor_last_error();
...
Compile with:
```bash
gcc -o example example.c -lintsoc_transactor -L./zig-out/lib
```

### From Idris2

```idris
import INTSOC_TRANSACTOR.ABI.Foreign
...
```rust
#[link(name = "intsoc_transactor")]
extern "C" {
    fn intsoc_transactor_init() -> *mut std::ffi::c_void;
    fn intsoc_transactor_free(handle: *mut std::ffi::c_void);
    fn intsoc_transactor_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = intsoc_transactor_init();
        assert!(!handle.is_null());

        let result = intsoc_transactor_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libintsoc_transactor = "libintsoc_transactor"

function init()
    handle = ccall((:intsoc_transactor_init, libintsoc_transactor), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:intsoc_transactor_process, libintsoc_transactor), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:intsoc_transactor_free, libintsoc_transactor), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/intsoc_transactor.h
   ```
